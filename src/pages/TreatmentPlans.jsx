...
import {
  GOAL_STATUSES, GOAL_STATUS_COLORS, DOMAINS,
  INTERVENTION_TEMPLATES, FREQUENCY,
} from '../data/treatmentPlansData';
import { useTreatmentPlans } from '../hooks/useTreatmentPlans';

export default function TreatmentPlans() {
  const { currentUser } = useAuth();
  const { patients } = usePatient();  // still needed for the patient selector in createPlan
  ...
  const updateGoalStatus = (planId, goalId, status) => {
    ...
    addNotification({ type: 'success', title: 'Goal Updated', message: `Status changed to ${status}` });
  };
  ...
  const addGoalToPlan = (planId) => {
    ...
    addNotification({ type: 'success', title: 'Goal Added', message: `${newGoal.domain} goal added to plan` });
  };
  ...
  const createPlan = () => {
    ...
    addNotification({ type: 'success', title: 'Treatment Plan Created', message: `Plan created for ${patient.firstName} ${patient.lastName}` });
  };
