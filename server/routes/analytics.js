...
router.get('/summary', async (req, res) => { const totalPatients = await db.prepare('SELECT COUNT(*) as count FROM patients').get().count;
  ...
  const todayAppts = await db.prepare('SELECT COUNT(*) as count FROM appointments WHERE date = ?').get(today).count;
  const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
  const weekAppts = await db.prepare('SELECT COUNT(*) as count FROM appointments WHERE date BETWEEN ? AND ?').get(weekAgo, today).count;
  ...
  const providerWorkload = await db.prepare(`SELECT provider, COUNT(*) as count, COUNT(CASE WHEN date = ? THEN 1 END) as today FROM appointments GROUP BY provider`).all(today);
  ...
  // Medication stats
  const totalMeds = await db.prepare('SELECT COUNT(*) as count FROM medications').get().count;
  const activeMeds = await db.prepare("SELECT COUNT(*) as count FROM medications WHERE status = 'Active'").get().count;
  const controlledMeds = await db.prepare('SELECT COUNT(*) as count FROM medications WHERE is_controlled = 1').get().count;
...
router.get('/patient/:id', async (req, res) => { const pid = req.params.id;
  ...
  const appts = await db.prepare('SELECT date, type, status FROM appointments WHERE patient_id = ? ORDER BY date DESC LIMIT 10').all(pid);
