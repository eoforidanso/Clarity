...
router.get('/metadata', async (req, res) => { res.json({
    resourceType: 'CapabilityStatement', status: 'active', date: new Date().toISOString(), kind: 'instance', fhirVersion: '4.0.1', format: ['json'], rest: [{
...
// ── Helper: FHIR Bundle ───────────────────────────────
function bundle(type, entries, total) { return {
    resourceType: 'Bundle', type, total: total ?? entries.length, entry: entries.map(e => ({
...
function toFhirPatient(p) { return {
    resourceType: 'Patient', id: p.id, meta: { lastUpdated: p.updated_at || p.created_at },
    identifier: [
    ...
    address: p.address_street ? [{ use: 'home', line: [p.address_street], city: p.address_city, state: p.address_state, postalCode: p.address_zip,  }] : [],
    contact: p.emergency_contact_name ? [{ relationship: [{ text: p.emergency_contact_relationship }],
...
router.get('/Patient/:id', authenticate, async (req, res) => { const p = await db.prepare('SELECT * FROM patients WHERE id = ?').get(req.params.id);
...
function toFhirEncounter(e) { return {
    resourceType: 'Encounter', id: e.id, meta: { lastUpdated: e.updated_at || e.created_at },
    status: 'finished',
...
router.get('/Encounter', authenticate, async (req, res) => { const { patient, date } = req.query;
  let encounters;

  if (patient) { encounters = await db.prepare('SELECT * FROM encounters WHERE patient_id = ? ORDER BY date DESC').all(patient); } else { encounters = await db.prepare('SELECT * FROM encounters ORDER BY date DESC LIMIT 100').all(); }

  res.json(bundle('searchset', encounters.map(toFhirEncounter)));
});

router.get('/Encounter/:id', authenticate, async (req, res) => { const e = await db.prepare('SELECT * FROM encounters WHERE id = ?').get(req.params.id);
...
function toFhirVital(v) { const components = [];
  if (v.bp) {
  ...
  return { resourceType: 'Observation', id: v.id, status: 'final', category: [{ coding: [{ system: 'http://terminology.hl7.org/CodeSystem/observation-category', code: 'vital-signs', display: 'Vital Signs' }] }],
...
function toFhirAssessment(a) { return {
    resourceType: 'Observation', id: a.id, status: 'final', category: [{ coding: [{ system: 'http://terminology.hl7.org/CodeSystem/observation-category', code: 'survey', display: 'Survey' }] }],
    code: { text: a.tool },
...
router.get('/Observation', authenticate, async (req, res) => { const { patient, category } = req.query;
  const results = [];

  if (!patient) return res.json(bundle('searchset', []));

  if (!category || category === 'vital-signs') { const vitals = await db.prepare('SELECT * FROM vitals WHERE patient_id = ? ORDER BY date DESC').all(patient);
    results.push(...vitals.map(toFhirVital)); }

  if (!category || category === 'survey') { const assessments = await db.prepare('SELECT * FROM assessments WHERE patient_id = ? ORDER BY date DESC').all(patient);
...
function toFhirCondition(p) { return {
    resourceType: 'Condition', id: p.id, clinicalStatus: { coding: [{ system: 'http://terminology.hl7.org/CodeSystem/condition-clinical', code: p.status?.toLowerCase() === 'active' ? 'active' : 'resolved' }] },
    code: { coding: [{ system: 'http://hl7.org/fhir/sid/icd-10-cm', code: p.code, display: p.description }],
      text: p.description,
    },
...
router.get('/Condition', authenticate, async (req, res) => { const { patient } = req.query;
  if (!patient) return res.json(bundle('searchset', []));
...
router.get('/Condition/:id', authenticate, async (req, res) => { const p = await db.prepare('SELECT * FROM problems WHERE id = ?').get(req.params.id);
  if (!p) return res.status(404).json({ resourceType: 'OperationOutcome', issue: [{ severity: 'error', code: 'not-found' }] });
...
function toFhirMedicationStatement(m) { return {
    resourceType: 'MedicationStatement', id: m.id, status: m.status?.toLowerCase() === 'active' ? 'active' : 'stopped', medicationCodeableConcept: { text: m.name },
    subject: { reference: `Patient/${m.patient_id }` },
    effectivePeriod: { start: m.start_date },
    dosage: [{ text: m.sig || `${m.dose } ${ m.route } ${ m.frequency }`,
      route: { text: m.route },
...
router.get('/MedicationStatement', authenticate, async (req, res) => { const { patient } = req.query;
  if (!patient) return res.json(bundle('searchset', []));
...
function toFhirAllergy(a) { return {
    resourceType: 'AllergyIntolerance', id: a.id, clinicalStatus: { coding: [{ system: 'http://terminology.hl7.org/CodeSystem/allergyintolerance-clinical', code: a.status?.toLowerCase() === 'active' ? 'active' : 'inactive' }] },
...
router.get('/AllergyIntolerance', authenticate, async (req, res) => { const { patient } = req.query;
  if (!patient) return res.json(bundle('searchset', []));
