...
router.get('/:patientId/medications', async (req, res) => { const rows = await db.prepare('SELECT * FROM medications WHERE patient_id = ? ORDER BY status ASC, name ASC').all(req.params.patientId);
...
router.put('/:patientId/medications/:medId', async (req, res) => { const b = req.body;
...
router.delete('/:patientId/medications/:medId', async (req, res) => { await db.prepare('DELETE FROM medications WHERE id = ? AND patient_id = ?').run(req.params.medId, req.params.patientId);
