...
router.get('/:patientId/allergies', async (req, res) => { const rows = await db.prepare('SELECT * FROM allergies WHERE patient_id = $1 ORDER BY created_at DESC').all(req.params.patientId);
  res.json(rows.map(r => ({
...
router.delete('/:patientId/allergies/:allergyId', async (req, res) => { await db.prepare('DELETE FROM allergies WHERE id = $1 AND patient_id = $2').run(req.params.allergyId, req.params.patientId);
  res.json({ success: true });
});

// ─── PROBLEMS ─────────────────────────────────────────────────

router.get('/:patientId/problems', async (req, res) => { const rows = await db.prepare('SELECT * FROM problems WHERE patient_id = $1 ORDER BY created_at DESC').all(req.params.patientId);
  res.json(rows.map(r => ({
...
router.get('/:patientId/vitals', async (req, res) => { const rows = await db.prepare('SELECT * FROM vitals WHERE patient_id = $1 ORDER BY date DESC, time DESC').all(req.params.patientId);
  res.json(rows.map(r => ({
...
router.get('/:patientId/immunizations', async (req, res) => { const rows = await db.prepare('SELECT * FROM immunizations WHERE patient_id = $1 ORDER BY date DESC').all(req.params.patientId);
  res.json(rows.map(r => ({
...
router.get('/:patientId/assessments', async (req, res) => { const rows = await db.prepare('SELECT * FROM assessments WHERE patient_id = $1 ORDER BY date DESC').all(req.params.patientId);
  res.json(rows.map(r => ({
