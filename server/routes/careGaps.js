...
router.get('/', async (req, res) => {
  const patients = await db.prepare("SELECT * FROM patients WHERE is_active = 1").all();
  const gaps = [];
  ...
  for (const p of patients) {
    ...
    if (onAntipsychotic) {
      const lastMetabolic = await db.prepare("SELECT date FROM lab_results WHERE patient_id = ? AND (name LIKE '%Metabolic%' OR name LIKE '%Lipid%' OR name LIKE '%A1C%') ORDER BY date DESC LIMIT 1").get(pid);
    ...
    if (onMoodStabilizer) {
      const lastLevel = await db.prepare("SELECT date FROM lab_results WHERE patient_id = ? AND (name LIKE '%Lithium%' OR name LIKE '%Valproat%') ORDER BY date DESC LIMIT 1").get(pid);
