...
function issuePortalToken(patientId) {
  return jwt.sign(
...
router.post('/request-access', async (req, res) => {
  ...
  // Lock check
  if (patient.portal_locked_until && new Date(patient.portal_locked_until) > new Date()) {
...
router.post('/verify-otp', async (req, res) => {
  ...
  // Expiry check
  if (!patient.portal_otp_expires || new Date(patient.portal_otp_expires) < new Date()) {
  ...
  if (!valid) {
    const attempts = (patient.portal_otp_attempts || 0) + 1;
    if (attempts >= 5) {
...
router.get('/me', authenticatePortal, async (req, res) => {
  const patient = await db.prepare(
    `SELECT id, first_name, last_name, email, dob, gender, phone, cell_phone,
  ...
  res.json({
    ...
    address: [patient.address_street, patient.address_city, patient.address_state, patient.address_zip]
      .filter(Boolean).join(', '),
