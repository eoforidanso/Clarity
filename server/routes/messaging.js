...
router.get('/channels', async (req, res) => { const channels = await db.prepare('SELECT * FROM staff_channels ORDER BY name').all();
  res.json(channels.map(c => ({ id: c.id, name: c.name, type: c.type })));
});

// GET /api/messaging/channels/:channelId/messages
router.get('/channels/:channelId/messages', async (req, res) => { const { limit } = req.query;
...
router.post('/channels/:channelId/messages', async (req, res) => { const b = req.body;
  const id = uuidv4();
...
router.put('/messages/:messageId/reactions', async (req, res) => { const { reactions } = req.body;
...
router.get('/dm/:userId/messages', async (req, res) => { const me = req.user.id;
...
// Send a DM to :userId
router.post('/dm/:userId/messages', async (req, res) => { const { content } = req.body;
...
router.put('/dm/messages/:messageId/reactions', async (req, res) => { const { reactions } = req.body;
  const me = req.user.id;
...
router.get('/dm/unread-counts', async (req, res) => { const rows = await db.prepare(
