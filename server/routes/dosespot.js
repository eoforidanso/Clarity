...
router.get('/sso', authenticate, authorize('prescriber'), async (req, res) => {
  if (!isConfigured()) {
    return res.status(503).json({
      error: 'DoseSpot is not configured on this server.',
      code: 'NOT_CONFIGURED',
    });
  }
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (Type === 'PrescriptionStatus' && Data?.PrescriptionId) {
      // Map DoseSpot status to internal order status
      const statusMap = {
        1: 'Active',         // Entered
        2: 'Active',         // Printed
        4: 'Active',         // Sending
